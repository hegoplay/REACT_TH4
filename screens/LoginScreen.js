...
const LoginScreen = ({ navigation }) => {
  ...
  const login = async () => {
    ...
    console.log("login");
    if (check == true) {
      await context.getUser(username);

      if (context.value.typeUser == "user") navigation.navigate("MainScreen");
      else if (context.value.typeUser == "admin")
        navigation.navigate("ManagerScreen");
      console.log(context.value.typeUser);
    } else {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.form}>
        <Text style={[{ textAlign: "center" }, styles.textColor]}>
          Đăng nhập
        </Text>
        {/* tai khoan */}
        <View>
